...
| `Beetlebot Manual.docx` | Phase 1–2: workspace setup + turtlesim; Phase 3–5: hardware connect, bringup, visualisation, `scp` | **Lab 1** (+ overlaps into Lab 2's connect step) | `05-lab1-turtlesim-and-workspace.md` |
| `VEEROBOT BEETLE BOT LYRA.txt` | Arm/disarm, the movement-command cookbook, keyboard teleop guide | **Lab 2** | `06-lab2-beetlebot-movement.md` |
| `BEETLEBOT  Obstacle_avoid.txt` | Obstacle-avoidance node, **v1** (simple, single front window) | **Lab 3** | `07-lab3-obstacle-avoidance.md` |
| `BEETLEBOT Updated _Obstacle_avoid.txt` | Obstacle-avoidance node, **v2 / "Updated"** (front+left+right windows, timed reverse state machine) | **Lab 3** | `07-lab3-obstacle-avoidance.md`, and the full line-by-line code read in `02-ros2-concepts.md` §5 |

## 3. Study path, in the order you actually did the labs
...
- `01-linux-commands.md` — every terminal command used across all three labs, explained.
- `02-ros2-concepts.md` — the ROS 2 computation model, `colcon`/workspaces, `rclpy` node
  anatomy, and the full line-by-line obstacle-avoidance code read.
- `03-beetlebot-runbook.md` — the single reconciled procedure (useful as an exam-day
  quick-reference once you already know *why* each step exists).
- `04-ros2-official-docs.md` — curated links into the official ROS 2 Jazzy docs.

Next: `05-lab1-turtlesim-and-workspace.md`.
