"""
klm_menu.py — generic text-menu engine.

Copied verbatim from https://github.com/KL-Mithunvel/menu (klm_menu.py).
Do not edit here; refresh by re-copying from upstream.

A menu is a dict:
    {
      "menu": "Title shown in the banner",
      "name": "key used in menu_system",
      "options": [ [cmd, prompt, hotkey], ... ],
      "back_option": True/False,
      "back_to": "name of menu to go back to" (or None)
    }

If a cmd contains ':', the text after ':' is treated as another menu name and
that menu is presented. Otherwise present_menu returns (cmd, menu_name).
"""
...
def present_menu(menu_to_present, menu_system):
    ...
    :param menu_system: menu_system dict object
...
def print_banner(header, star="*", width=40):
    """
    Prints a banner - a heading sandwiched between two lines of stars.
